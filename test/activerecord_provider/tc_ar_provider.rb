class ActiveRecordProviderTest < Test::Unit::TestCase
  ...
  def test_get_record
    assert_nothing_raised { REXML::Document.new(@provider.get_record('oai:test/1')) }
    doc = REXML::Document.new(@provider.get_record('oai:test/1'))
    assert_equal 'oai:test/1', doc.elements['OAI-PMH/GetRecord/record/header/identifier'].text
  end
  
  def test_deleted
    DCField.update(5, :deleted => true)
    doc = REXML::Document.new(@provider.get_record('oai:test/5'))
  ...
  def test_from
    ...
    doc = REXML::Document.new(
      @provider.list_records(:from => Chronic.parse("January 1 2006"))
      )
    assert_equal 11, doc.elements['OAI-PMH/ListRecords'].to_a.size
